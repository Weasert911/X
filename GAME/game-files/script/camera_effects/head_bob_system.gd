...
## Enhanced Head Bob System
## Natural, velocity-based procedural animation with idle/walk/sprint states
...
var _current_state: float = 0.0  # 0.0 = idle, 1.0 = walk, 2.0 = sprint
...
func update(delta: float, speed: float, sprinting: bool, grounded: bool) -> void:
	"""Update head bob based on player state."""
	if not enabled:
		return
	...
	# Calculate bob parameters based on current state
	...
	# Emit footstep signal if enabled
	if footstep_sync_enabled and grounded and speed > 0.1:
		# Check if we crossed zero going positive (foot down)
...
func get_offset() -> Vector3:
	...
	# Calculate bob offset (combined vertical and horizontal)
...
func _get_current_amount() -> float:
	"""Get current bob amount based on state."""
	if _current_state <= 1.0:
